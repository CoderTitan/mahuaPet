...
class CommentReplyItem extends StatefulWidget {
  ...
  final int userId;
  CommentReplyItem({required this.replyLists, required this.userId, Key? key})
      : super(key: key);
...
class _CommentReplyItemState extends State<CommentReplyItem> {
  ...
  List<Widget> renderUserInfoItems(CommentReplyVo model) {
    ...
    Widget time = Text(model.replyPublishTime,
        style: TextStyle(fontSize: 11.px, color: TKColor.color_999999));
    commentList.add(SizedBox(width: 4.px));
    ...
    return Container(
        ...
        child: Text.rich(TextSpan(children: [
          WidgetSpan(
              child: Text('回复',
                  style:
                      TextStyle(fontSize: 13.px, color: TKColor.color_666666)),
              alignment: PlaceholderAlignment.middle),
          ...nickName.runes.map((rune) {
            return WidgetSpan(
                child: Text(String.fromCharCode(rune),
                    style: TextStyle(
                        fontSize: 14.px, color: TKColor.color_526e94)),
                alignment: PlaceholderAlignment.middle);
          }).toList(),
          ...model.commentInfo.runes.map((rune) {
            return WidgetSpan(
                child: Text(String.fromCharCode(rune),
                    style: TextStyle(
                        fontSize: 13.px, color: TKColor.color_333333)),
                alignment: PlaceholderAlignment.middle);
          }).toList(),
        ])));
  }
        ...
        child: Text.rich(TextSpan(children: [
          WidgetSpan(
              child: Text(_showAll ? '收起' : '展开所有',
                  style:
                      TextStyle(fontSize: 13.px, color: TKColor.color_526e94)),
              alignment: PlaceholderAlignment.middle),
          WidgetSpan(
              child: Icon(Icons.arrow_drop_down,
                  size: 14.px, color: TKColor.color_526e94),
              alignment: PlaceholderAlignment.middle),
        ])),
      ),
      onTap: () {
