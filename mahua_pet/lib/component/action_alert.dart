              ...
              child: ListView.builder(
                ...
                itemBuilder: (ctx, index) {
                  final backColor = colorArray.length > 0
                      ? colorArray[index]
                      : (isNight ? TKColor.color_151b26 : TKColor.white);
                  final textColor = (colorArray.length > 0 || isNight)
                      ? TKColor.white
                      : TKColor.color_1a1a1a;
                  return GestureDetector(
                    child: Container(
                      color: backColor,
                      child: Text(titleArray[index],
                          style: TextStyle(fontSize: 14.px, color: textColor)),
                    ),
                    onTap: () {
                      Navigator.of(context).pop();
                      onTap(index);
...
class _MaterialAlert extends StatelessWidget {
  ...
  List<Widget> renderActions(BuildContext context) {
    ...
    Widget leftItem = TextButton(
      child: Text(leftTitle ?? '取消',
          style: TextStyle(fontSize: 16.px, color: TKColor.color_999999)),
      onPressed: () {
        Navigator.of(context).pop();
        if (leftPressed != null) {
          leftPressed!();
    ...
    if (rightTitle != null) {
      Widget item = TextButton(
        child: Text(rightTitle ?? '',
            style: TextStyle(fontSize: 16.px, color: TKColor.main_color)),
        onPressed: () {
          Navigator.of(context).pop();
          if (leftPressed != null) {
            rightPressed!();
